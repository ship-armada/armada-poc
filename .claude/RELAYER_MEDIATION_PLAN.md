...
**Status: IMPLEMENTED (branch `iskay/gasless-shield-permissionless`), pending manual e2e validation.** Landed as commits C1 `89934da4` (hub wrapper), C2 `d5fc91d2` (cross-chain + deploy-script), C3 `d081f24e` (interface + v1 relayer). Foundry + interface + relayer unit tests green; end-to-end (real CCTP two-note mint) validated manually via the interface. Full plan: `.context/gasless-shield-permissionless-plan.md`; v2-relayer port: `.context/gasless-shield-v2-relayer-port.md`. Two v1 simplifications from the original plan: (1) **no fee gross-up** — the fee note is charged the pool shield fee, so the relayer nets marginally below the quote (documented minor under-reimbursement; exact gross-up deferred); (2) **gasless shields carry `integrator = address(0)`** to keep the fee-note net predictable. Supersedes Phase B's trust model. Phase B shipped the `onlyRelayer` wrappers as the POC-grade mitigation; Phase C removes that gate and brings gasless shield fully in line with the permissionless, shielded-fee model every other op already uses. This is the "typed-data binding" future-work item referenced under Phase B's PR table.
...
- **Fee accuracy — hardcoded gas estimate + hardcoded ETH price.** Today's quote uses a fixed `500_000` gas constant per op and a fixed `ETH_USDC_PRICE=2000` env var. Both are conservative-but-blunt: real `transact()` consumption is closer to 300–400k for the typical 1×2 shape, and the ETH price is static while the live rate moves. Result is a quote that's usually too high (~30% headroom from the gas estimate alone) and occasionally stale on the ETH side. **Remediation (planned, optional polish — not blocking):** (1) Background job in the relayer that simulates representative tx shapes (1×2, 2×2, 2×3, etc.) against the live hub chain on a configurable cadence, caches the result per shape, and feeds `gasEstimate` per op tier — see also the `getStartScanningBlock`-style cursor pattern we already use for Iris. (2) Replace the hardcoded `ETH_USDC_PRICE` with a live read from the Chainlink ETH/USD feed on the hub chain (Sepolia: `0x694AA1769357215DE4FAC081bf1f309aDC325306`; mainnet: `0x5f4eC3Df9cbd43714FE2740f5E3616155c5b8419`). One `latestRoundData()` call cached for ~60s with a staleness guard that falls back to the env-configured value when the feed is too old or unavailable. Combined effort ~2 days; no `/fees` API surface change so frontend stays untouched. Reduces "fee feels high" pressure significantly and removes a class of operator-error surface (forgetting to update `ETH_USDC_PRICE` as the market moves).
