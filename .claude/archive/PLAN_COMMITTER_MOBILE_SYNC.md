...
- **Phase 4 — Committer header + wiring.** Committer renders `AppHeader` (crowdfund-shared) via `AppShell`, NOT `@armada/ui Header`. `AppShell`/`AppHeader` are shared with the (deprecated) observer.
  - **Locked decisions:**
    1. **Restyle the shadcn `Sheet` full-screen** (keep Radix focus-trap/scroll-lock/Escape) rather than a custom overlay.
    2. **Observer is deprecated** → no opt-in gating needed; change `AppHeader` directly for the committer's benefit (observer inherits the new menu, acceptable).
    3. **Richer wallet block** in the menu (provider icon + address + copy + disconnect + balance) wired to **RainbowKit** hooks + `@web3icons` (already a dep) — "feasible within reason".
    4. **Include the `Participate` card** inside the mobile menu (designer parity).
