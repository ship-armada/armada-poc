...
- Missing `onlyDelegatecall` guards on pool modules
...
Issues are tracked on Codeberg at `codeberg/armada/railgun-demo`. The `tea` CLI is available for creating and editing issues.
...
2. **Annotation**: Always prepend this line as the first line of the issue description:
   ...
   ```
3. Use `tea issues create --remote codeberg` and `tea issues edit --remote codeberg` for all issue operations.
