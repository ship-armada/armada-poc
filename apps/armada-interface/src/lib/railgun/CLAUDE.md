...
| `quickSync.ts` | The `QuickSyncEvents` callback wired into the engine — paginates `GET /v1/quick-sync/:hubChainId` on the relayer-v2 watcher and returns `AccumulatedEvents`. Degrades to empty (→ engine slow scan) when `indexerUrl` is unset, on a non-hub chain, a non-V2 txid version, or any fetch/parse/validation failure. Never throws. Emits a `railgun.quicksync` telemetry line per attempt (`served` / `no-indexer` / `fell-back`) — grep the console to verify activation vs fallback. | Working |
| `nullifierCrossCheck.ts` | WI-5 safety net: after a scan, queries the hub PrivacyPool's `nullifiers(...)` for the wallet's own unspent notes (`wallet.TXOs`). Catches a watcher that omits a `Nullified` event (which merkleroot validation can't see). Fails open on RPC error. Bridge hook: `hooks/useNullifierCrossCheck.ts`; gate: `hooks/useSpendableSyncGate.ts`. | Working |
