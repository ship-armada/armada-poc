...
import { IUnshieldCallback } from "../interfaces/IUnshieldCallback.sol";
...
contract RailgunLogic is Initializable, OwnableUpgradeable, Commitments, TokenBlocklist, Verifier {
   ...
   * @notice Transfers tokens to contract and adjusts preimage with fee values
   * @param _note - note to process
   * @param _originalSender - the address that initiated the transaction (for callback)
   */
  function transferTokenOut(CommitmentPreimage calldata _note, address _originalSender) internal {
    // validateTransaction and accumulateAndNullifyTransaction functions MUST be called
    // in that order BEFORE invoking this function to process an unshield on a transaction
    // else reentrancy attacks are possible
    ...
    if (_note.token.tokenType == TokenType.ERC20) {
      ...
      // POC ONLY: Call callback if recipient is a contract implementing IUnshieldCallback
      // This allows single-transaction unshield-to-bridge flows
      if (recipient.code.length > 0) {
        try IUnshieldCallback(recipient).onRailgunUnshield(
          address(uint160(_note.token.tokenAddress)),
          base,
          _originalSender
        ) {} catch {
          // Silently ignore if callback fails or is not implemented
          // This maintains backwards compatibility with existing contracts
        }
      }
    } else if (_note.token.tokenType == TokenType.ERC721) {
      // ERC721 token
