// ABOUTME: Derives participant-table rows from the event-derived CrowdfundGraph (no RPC).
// ABOUTME: Ported from the admin app's useParticipants for the committer Observe view; candidate to hoist into @armada/crowdfund-shared if a third consumer appears.

import { useMemo } from 'react'
import { type CrowdfundEvent, buildGraph, HOP_CONFIGS } from '@armada/crowdfund-shared'
...
export function useParticipants(events: CrowdfundEvent[]): ParticipantRow[] {
  return useMemo(() => {
    ...
    // Allocations + refund claims come from events, not the graph nodes.
