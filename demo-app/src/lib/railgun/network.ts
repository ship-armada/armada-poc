...
import { loadDeployments, getHubChain } from '../../config';
...
function patchNetworkConfig(
  ...
  const hardhatNetwork = networkConfig?.Hardhat ?? networkConfig?.[NetworkName.Hardhat];
...
async function patchHardhatConfig(): Promise<void> {
  ...
  const patchedPrimary = patchNetworkConfig(
    NETWORK_CONFIG as unknown as Record<string, any>,
    privacyPool
  );
