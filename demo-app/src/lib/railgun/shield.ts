 ...
 *
 * NOTE: This uses SDK components directly without requiring full engine init.
 * The engine initialization requires downloading large WASM artifacts which
 * is complex to set up. For shield operations, we only need the cryptographic
 * primitives which are available without the engine.
 */
