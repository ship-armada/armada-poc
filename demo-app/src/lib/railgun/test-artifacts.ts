 ...
 */
async function loadCircuitArtifact(config: { nullifiers: number; commitments: number }): Promise<Artifact> {
  const dirName = configToDirName(config);
  const basePath = `/artifacts/${dirName}`;

  console.log(`[test-artifacts] Loading ${config.nullifiers}x${config.commitments}...`);
  ...
  console.log(`[test-artifacts] Loaded ${config.nullifiers}x${config.commitments}: zkey=${(zkey.length / 1024 / 1024).toFixed(2)}MB, wasm=${(wasm.length / 1024 / 1024).toFixed(2)}MB`);
  ...
  for (const config of CIRCUIT_CONFIGS) {
    try {
      ...
      console.log(`[test-artifacts] Artifact ${variant} details:`, {
        zkeyLength: artifact.zkey instanceof Uint8Array ? artifact.zkey.length : 'not Uint8Array',
        wasmLength: artifact.wasm instanceof Uint8Array ? artifact.wasm.length : 'not Uint8Array',
      ...
      console.error(`[test-artifacts] Failed to load artifact ${config.nullifiers}x${config.commitments}:`, error);
      throw error;
    }
  }

  console.log(`[test-artifacts] Successfully loaded ${CIRCUIT_CONFIGS.length} test artifacts`);
 ...
 */
export function isCircuitLoaded(nullifiers: number, commitments: number): boolean {
