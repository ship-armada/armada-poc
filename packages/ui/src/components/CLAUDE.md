...
- **`tokens.css` `[data-theme="light"]` block** — populated with the mockup's real light values (faithful). NOTE: the `armada-tokens.json` reference copy is **not yet mirrored** for the light block — a tracked sync debt.
