...
- **`WalletPillMenu` `shieldedAddress?: string` two-row trigger** — same precedent. When supplied, the pill trigger renders a second row (lavender 0zk…) under the EVM address and swaps the wallet-provider icon for `ArmadaSymbol` (the trigger is now branded as the user's Armada identity, not their wallet provider). Crowdfund consumers omit the prop and keep the original single-row layout.
- **`Button` `leadingIcon?: ReactNode` slot** — same precedent. Mockup buttons are label-only or label+trailing-arrow. `armada-interface`'s "Connect Wallet" pill renders a leading `LogIn` (lucide-react) icon to read as an entry-action button. Added alongside the existing `showIcon` trailing-arrow prop; mirrored CSS class `.leading.<size>` applies the same left-padding token as `.icon.<size>` does on the right. Crowdfund consumers omit the prop and get the original geometry.
- **`ArmadaLogo` `variant: 'mono'`** — same precedent. Third variant alongside the re-ported `full`/`mark`: renders the full layout with every flotilla path in `currentColor` (via `ArmadaMark tone='white'`, no gradient defs) so the parent's CSS color drives the glyph. Used by `armada-interface`'s dashboard header. Crowdfund consumers use `full`/`mark` and are unaffected.
