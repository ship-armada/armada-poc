...
- **S-M4 — NOT STARTED. Riskiest remaining + needs a design decision.** Threads `markWaiting` + `approveTxHash`/`approveSkipped` artifacts through the funds-critical shield + shield-xchain handlers (DIRECT and GASLESS paths), then renders the built-but-unused `WalletConfirmList`. Feasibility confirmed: the waiting↔active flip works via `advance(record, record.stage, patch)` (advance sets active on a non-terminal stage). **Open question: how does WalletConfirmList render on the GASLESS path** (no on-chain approve; the only wallet prompt is the EIP-2612 permit during build-proof)? Needs its own handler tests (assert markWaiting + artifacts at each prompt, both paths). Do this FRESH, not at the tail of a long session.
- **S-L1 — decision made (A: curate error-ABI list), but scope TBD.** Contracts define **~61 custom errors**; curating all is drift-prone. Recommend a focused list of errors reachable from the direct user-wallet writes (PrivacyPool.shield, PrivacyPoolClient.crossChainShield, the yield adapter, transact wrapper). Compute selectors via viem `toFunctionSelector`/`AbiError`; decode in revert.ts; unknown selectors fall through to current behavior.
