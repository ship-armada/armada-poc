...
8. **Redeem-before-sweep (issue #254):** `ArmadaRedemption.redeem()` has two layered mitigations:
   ...
   - **`REDEMPTION_DELAY` (7 days):** gates all redemptions until 7 days after `triggerTime`, providing a social-coordination window for sweeps to complete.

   Residual risk: **partial-sweep forfeiture** — if some tokens are swept and others are not when a user redeems, they receive a share of swept assets but forfeit their claim on unswept ones. The unpaid share then flows pro-rata to later redeemers. The 7-day delay mitigates this by giving all sweeps time to complete, but does not guarantee they do. Operational: rely on coordinators/relayers to batch-sweep during the delay window, and frontends to warn if treasury balances remain.

   **Forgotten-token forfeiture (issue #256, accepted residual):** the variant where a token *is* swept and available but the redeemer omits it from `tokens[]`. The redemption succeeds, ARM is locked, and the omitted share is silently and irreversibly forfeited to later redeemers. ETH cannot be forgotten (`ethRecipient` must be non-zero when the pool holds ETH) and listing an unswept token reverts (`zero share for token`), but omitting an available token has no on-chain guard. Decision (2026-07): accept on-chain (no change to the immutable `redeem()` interface) and mitigate operationally:
   - **`RedemptionRouter`** (`contracts/periphery/RedemptionRouter.sol`) — pre-built contingency periphery, deployed only during the 7-day window with the complete swept-token list baked in at construction (`scripts/deploy_redemption_router.ts` builds it from `TokenSwept` events). `redeemAll(armAmount, recipient)` claims every asset in one call, removing the footgun for UI and direct callers alike. Kept compiling and tested in CI (`test-foundry/RedemptionRouter.t.sol`, `test/winddown_redemption_integration.ts`) so it cannot rot before it is needed.
   - **Wind-down runbook** (`docs/winddown-redemption-runbook.md`) — window checklist: sweep everything, deploy + verify the router, publish a signed manifest (router address, complete token list, calldata template for direct callers), monitor `Redeemed` events for incomplete token lists.
