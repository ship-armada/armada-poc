...
async function main() {
  ...
  // 5b. Register governor on VotingLocker (enables unlock cooldown)
  console.log("5b. Registering governor on VotingLocker...");
  await (await votingLocker.setGovernor(governorAddress, nm.override())).wait();
  console.log(`   VotingLocker.governor = ${governorAddress}`);
