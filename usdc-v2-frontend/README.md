...
- **Armada Relayer** running on `localhost:3001` — required for cross-chain shield operations. Start it from the root `poc/` directory with `npm run armada-relayer` (not `npm run relayer`, which is the legacy CCTP-only relay without an HTTP API).
