 ...
 * Same pattern as lend but with vault shares as input and USDC as output.
 */
...
export interface YieldAdaptRedeemProofResult {
  transaction: {
    to: string
    data: string
    value: bigint
...
export async function generateYieldAdaptRedeemProof(
  ...
  const relayShieldRequests = await RelayAdaptHelper.generateRelayShieldRequests(
    shieldRandom,
    [{ tokenAddress: usdcAddress, recipientAddress: railgunAddress }],
    [],
  )
