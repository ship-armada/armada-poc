          ...
          req.on('end', async () => {
            try {
              ...
              if (!v3File && !v2File) {
                ...
                res.end(
                  JSON.stringify({
                    error: `No deployment for chainId: ${chainId}`,
                  }),
                )
                return
              }
              ...
              const v3Path = v3File
                ? path.resolve(__dirname, `../deployments/${v3File}.json`)
                : null
              const v2Path = v2File
                ? path.resolve(__dirname, `../deployments/${v2File}.json`)
                : null
              ...
              if (!deploymentPath) {
                ...
                res.end(
                  JSON.stringify({
                    error: `Deployment file not found for chainId: ${chainId}`,
                  }),
                )
                return
              }
              ...
              if (!faucetAddress) {
                ...
                res.end(
                  JSON.stringify({
                    error: 'Faucet not deployed on this chain',
                  }),
                )
                return
              }
              ...
              console.log(
                `[faucet] Dripped to ${address} on chain ${chainId} (1000 USDC + 1 ETH)`,
              )
